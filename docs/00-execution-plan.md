...
not replace the other `docs/0N-*.md` files — it points at them.

Ticket numbers below are GitHub issue numbers. `#34` (`B0`) and `#35` (`C0`)
are new tickets opened by this gap analysis; every other ticket already
existed.
...
|---|---|---|
| Configure the server as an MCP connection | B1 (#9) | `agent/config.py` + `python -m agent.run --discover-only` output |
| Discover and call ≥1 of its tools | B1 (#9), B0 (#34) | `docs/playwright-tools.json` dumped from the live server |
...
| Real public, no-login page actually confirmed (not the `.env.example` placeholder) | **B0 (#34) — new** | real URL in README, live navigate+snapshot sample under `fixtures/playwright/`, selector verified against the real DOM |
...
| Fixtures preserve the real parsing path | C6 (#21) | explicit statement + proof in `fixtures/README.md` |
...
| 2 | Show the agent discovers both connections | B1 (#9), C5 (#20) | `--discover-only` output |
...
|---|---:|---|
| 1. MCP architecture and protocol correctness | 25 | A1/A2 (done), B1 (#9), B0 (#34), D1 (#24) |
...
| Both MCP connections can be called successfully | B1 (#9), B0 (#34) | discover + one real call each |
| Agent incorporates both servers into its flows | B3, B4 (#11, #12) | `agent/graph.py` |
...
| Hard-coded demo answers presented as tool results | Fixtures replay through the same parser as live markup (C6 DoD requires an explicit statement + proof); the optimizer and comparator compute from data, no LLM inside a tool (A4, A5). |
...
inserted where they actually block work:

```
day 1   B0            (parallel with whatever A/C work remains)
day 2   B1, B2         C1, C7
day 3   B3             C6
day 4   B4             C2, C3
day 5   B5, B6         C4
...
Critical path: **B0 → B2 → B3 → B4 → D1 → D2 → D4**, with **C0 → C5** feeding
...
|---|---|---|
| `laptop-a` | Track A (`mcp_servers/`) — closed. Available for **Track D** (integration/rehearsal) once B4 lands, or for whichever track is free per `bash scripts/next_ticket.sh`. | B0 (#34) if free right now — it has no dependencies and unblocks the rest of Track B. |
| `laptop-b` | Track B (`agent/`) | B1 (#9) is already claimed and in progress; B0 (#34) can run in parallel since it touches only `.env.example`/README/fixtures, not `agent/`. |
...
C → whichever machine is free. B0 and C0 stay inside their existing tracks'
directories (`.env.example`/`README.md`/`fixtures/` for B0; `docs/` for C0),
so they do not create a new cross-track conflict surface.
...
2. **Minimum-condition rule fully satisfied.**
   `python scripts/verify_mcp.py` shows 5 tools; `python -m agent.run --discover-only` shows both connections; `agent/graph.py` shows both servers in the same flow.
...
4. **B0 and C0 are both closed before C5/D2 are attempted.**
   Otherwise the demo script is written against an unconfirmed target page and an undecided defence format — the two gaps this plan exists to close.
...
|---|---|---|
| The real timetable page (once B0 picks it) turns out to require login or JS interaction Playwright's read-only navigate/snapshot can't reach. | Medium | Fall back to a different public page section that is genuinely public (e.g. a published group timetable rather than a personalized one); worst case, treat the recorded fixture as the primary demo path and say so honestly — the assignment permits demonstrating failure/offline mode as long as it is not presented as if it were live. |
| University changes the page layout again between B0 and the defence, breaking the selector. | Medium — the whole project already assumes this happens mid-semester. | This is exactly what `detect_timetable_changes` and the v1→v2 fixture switch are built to demonstrate; re-verify the selector once, right before recording (part of C0's dry run), and keep the offline fallback current. |
| `laptop-a` and `laptop-b` pick up B0 simultaneously since it sits outside `agent/` proper and looks track-agnostic. | Low, but the failure mode (duplicate work, merge conflict) is annoying. | B0 is filed under `track:B-agent`; whoever runs `/take` first on #34 owns it — the claim bot already prevents a second claim. |
| C0's format decision gets deferred until the last week, and the dry run reveals the script overruns 15 minutes. | Medium | C0 is scheduled early (day 1–2 of the C-track work) specifically so a timing problem surfaces while there is still time to trim `docs/05-demo-checklist.md`, not the night before submission. |
| A rubric gap surfaces late because `docs/07-rubric-selfcheck.md` was filled in optimistically rather than with real evidence. | Medium | C8 is explicitly the last C-track ticket before D-track rehearsal; D1's DoD requires "every deviation from the script filed as a ticket," which is the safety net if C8 missed something. |
